# Long-Form Articles & Position Papers

This folder contains **longer-form thinking pieces** on 3D printing market dynamics, open-source philosophy, and the future of scavenger-friendly hardware design. These are essays and position papers, not technical specs.

## Contents

- **age-of-the-appliance.md** — The shift from DIY tooling to consumer appliances
  - Market consolidation around easy-to-use commercial systems
  - Where scavenger projects fit in the appliance era
  - Philosophy: "Tractor with a Racecar Brain"

- **reclaiming-the-reprap-legacy.md** — Vision piece for Amalgam's target release
  - RepRap heritage and Amalgam's connection to it
  - Why threaded-rod frame design remains relevant
  - Market positioning in 2027

- **reconfiguration-of-3d-printing-market.md** — Market analysis and trends
  - 20-year arc from RepRap to modern appliance printers
  - Why Amalgam exists as an alternative path

- **core-engineering-truths.md** — Philosophical foundations
  - Core principles that have survived 20 years of 3D printing
  - Why certain design choices are timeless

## Purpose

These essays provide context for:
- **Project philosophy** — why Amalgam exists
- **Market positioning** — who Amalgam is for
- **Heritage acknowledgments** — crediting decades of open-source work
- **Design philosophy** — understanding the "Tractor" ethos

They are **not** technical specifications. For design rationale, see `docs/adr/`.

## Related Documentation

- **Technical Decisions** → See `docs/adr/` (structured design choices)
- **Deep Dives** → See `docs/deep-dives/` (design exploration)
- **Philosophy** → See `PHILOSOPHY.md` (core project values)

---

*These are think pieces meant to provide context. For technical guidance, see guides/ and ADRs.*
